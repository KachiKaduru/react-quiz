  ...
  switch (action.type) {
    ...
    case "requestLoan":
      return {
        ...
        balance: activeLoan ? state.balance : state.balance + action.payload,
      };
    case "payLoan":
      return {
        ...state,
        loan: activeLoan ? state.loan - action.payload : state.loan,
        balance: activeLoan ? state.balance - action.payload : state.balance,
      };
    case "closeAccount":
      return {
        ...state,
        isActive: state.balance === 0 && state.loan === 0 ? !state.isActive : state.isActive,
      };
    default:
...
export default function ReducerBankApp() {
  const [state, dispatch] = useReducer(reducer, initialState);
  const { balance, loan, isActive } = state;

  return (
    <div className="reducer-app">
